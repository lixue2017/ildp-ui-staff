    ...
    >
      <template v-slot:content>
        <div v-if="$refs.dialogRef && $refs.dialogRef.dialogVisible">
          ...
          <ComTable
            ref="bgTableRef"
            :columnConfig="tablePackageColumn()"
            :columnData="[]"
...
import { postReceiptAddPackage } from "@/api/warehouse/process";
...
import { packageFormCols, formRules, tablePackageColumn } from "./model";
...
export default {
    ...
    return {
      ...
      packageFormConfig: {
        ...
        successFunction: () => {
          ...
          this.packageDialogObj.successFn && this.packageDialogObj.successFn();
        },
        afterMounted: () => {
          setTimeout(() => {
            this.handleAddSku();
  ...
  methods: {
    ...
    handleFormRequest(fData) {
      const { customerId, orderId, receivingSource } = this.packageDialogObj;
      return postReceiptAddPackage({
        ...fData,
        customerId,
        orderId,
        receivingSource,
        items: this.mixinTableDataArr.map((e) => ({
          cpmId: e.cpmId,
          num: e.num,
        })),
      });
