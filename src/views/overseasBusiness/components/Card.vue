      ...
      <el-col :span="24 / config.length" :key="i">
        ...
        >
          <div
            ...
            :class="`iconfont ${
              item.icon ||
              item.iconObj[formData.directClearanceOrderDetail.transportMode]
            }`"
          ></div>
          <div class="card-right">
            <div class="card-line" v-for="l in item.list" :key="l.prop">
              ...
              <span
                v-if="
                  l.dictVal &&
                  (formData[l.dictVal] || formData[l.dictVal] === 0)
                "
                class="tag-green-color"
                >{{
                  getDictObj(dictData.freightType, formData[l.dictVal])
                }}</span
              >
            </div>
          </div>
