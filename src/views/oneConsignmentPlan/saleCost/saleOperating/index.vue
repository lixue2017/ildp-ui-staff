...
import {
  getSaleOptPriceList,
  enableDisableSaleOptPrice,
  deleteSaleOptPrice,
  getcostSaleOptPriceUp,
  putCostSaleOptPriceEnableDisable,
  putCostSaleOptPriceUpdEnablingTime,
  putCostSaleOptPriceCancelUp
} from "@/api/oneConsignmentPlan/saleCost";
import {getNowDate} from "@/utils/index.js"
export default {
  name: "SaleOperatingList",
  ...
  data() {
    return {
      ...
      tableConfig: {
        ...
        httpRequest: getSaleOptPriceList,
      },
  ...
  methods: {
    ...
    handleView(row) {
      ...
      this.$router.push({
        path: "/oneConsignmentPlan/saleCost/saleOperatingDetail",
        query: {
          id: row.id,
    ...
    handleInterdict(rows) {
      ...
      if (id) {
        ...
        msgBox(this, { title, className, msg }, () => {
          this.comRepeatedRequest(this, putCostSaleOptPriceEnableDisable, { id}).then(
            () => {
    ...
    handleDelete(rows) {
      ...
      if (id) {
        ...
        const className = "el-icon-warning text-warning";
        msgBox(this, { title, className, msg }, () => {
          this.comRepeatedRequest(this, deleteSaleOptPrice, { id }).then(() => {
    ...
    handleUpgradation(row) {
      ...
      this.$refs.confirmDlgRef?.upgrade().then(() => {
        getcostSaleOptPriceUp(id).then((res) => {
          const { id } = res?.data || {};
    ...
    handleChangeState(row={},type){
      ...
      const {customFormConfig,dialogConfig={},httpRequest,tips=undefined}={
        0:{//启用
          ...
          httpRequest:putCostSaleOptPriceEnableDisable,
          dialogConfig:{
            title:"销售操作费-启用"
          },
          tips:"启用成功！"
        ...
        1:{//修改生效时间
          customFormConfig:diyCustomDialogCols,
          httpRequest:putCostSaleOptPriceUpdEnablingTime,
          dialogConfig:{
            title:"修改生效时间"
          },
        },
    ...
    handleCancel(row){
      ...
      const msg = "取消操作不可逆！";
        ...
        const className = "el-icon-warning text-warning";
        msgBox(this, { title, className, msg }, () => {
          this.comRepeatedRequest(this, putCostSaleOptPriceCancelUp, id).then(
            () => {
      ...
      switch (type) {
        ...
        case "enable":
          ...
          if(state==2){
            ...
            putCostSaleOptPriceEnableDisable(params).then((res)=>{
              this.mixinkeepTable()
            })
          }else{
