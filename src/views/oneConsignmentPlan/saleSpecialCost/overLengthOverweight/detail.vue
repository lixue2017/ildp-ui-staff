...
<script>
import { getSpecialRuleInfo } from "@/api/oneConsignmentPlan/saleSpecialCost";
...
export default {
  ...
  created() {
    this["dictionary/getDictionary"]("odaConfig").then(() => {
      const { costSpecialChargesRuleState = [] } = this.dictionary.dicsData || {}
  ...
  methods: {
    ...
    getDetails(newId) {
      ...
      getSpecialRuleInfo(id).then((res) => {
        const {
          fastening,
