              ...
              <template v-slot:customRow="{ slotProps }">
                <template v-if="slotProps.prop === 'type'">
                  <span
                    ...
                    v-if="slotProps.row.type === 2"
                  >
                    是
                  </span>
                  <span v-else-if="slotProps.row.type === 1">否</span>
                  <span v-else>--</span>
                </template>
              </template>
            </ComTable>
...
export default {
        ...
        statusBtnList: [
          {
            text: "修改",
            showStatus: [0],
            handleClick: (row) => this.handleDetails(row, "1"),
          },
          {
            text: "复制",
            showStatus: [1, 2],
  ...
  methods: {
    ...
    handleUpdate({ id }, status) {
      // 状态：0-草稿，1-启用，2-禁用
      putCostPartitionScheme(id).then(() => {
    ...
    handleDelete(row) {
      customMessageBox(
        this,
        {
          msgTitle: "删除提示",
          msgTxtColor: "#FF6767",
          httpMsgRequest: () => delCostPartitionScheme(row.id),
        },
        () => {
