...
import {
  getBusinessRefundCheckDetail,
  exportBusinessRefundCheckDetail,
...
export default {
  ...
  data() {
    return {
      ...
      searchFormConfig: {
        formModel: {
          checkDate: [formatDate(new Date(new Date().getFullYear(), 0, 1), "-", "date"),formatDate(new Date().getTime(), "-", "date")],
        }, // 表单数据
      ...
      tableConfig: {
        ...
        httpRequest: (formModel) => {
          return new Promise((resolve) => {
            getBusinessRefundCheckDetail(formModel).then((res) => {
  ...
  // watch: {
  //   "searchFromData.businessCompany": {
  //     deep: true,
  ...
  //         customerId: newVal,
  //       };
  //       GETCurrencyConfigList(params).then((res) => {
  //         const { data } = res;
  //         this.currencyList = data[0].currencyList || [];
  //       });
  //     }
  ...
  methods: {
    ...
    getProfitStatementExport() {
      ...
      comFileDownload(this, exportBusinessRefundCheckDetail, params);
    },
  },
  components: {
