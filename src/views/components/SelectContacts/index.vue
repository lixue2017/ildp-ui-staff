<template>
  <div
    :class="[
      ...
      fieldItem.tagName &&
        (!Object.keys(contactsData).length || !contactsData.id) &&
        'padding-bottom-15',
    ]"
  >
    ...
    >
      <AutoComplete
        ...
        :fieldItem="{
          ...
          refreshRequest: contactsRefreshRequest,
        }"
        defaultKey="id"
      ...
      <div
        class="contacts-phone flex"
        v-if="Object.keys(contactsData).length && contactsData.id"
      >
    ...
    >
      <div
        :style="{ maxWidth: fieldItem.maxWidth || '320px', display: 'flex' }"
        v-if="!fieldItem.isHidePhone"
      >
      ...
      <div
        ...
        v-if="
          ...
          (!fieldItem.disable ||
            handleClearance(formModel.customsClearanceId)) &&
          !fieldItem.isHidePhone
        "
      >
        <span
          :class="
            fieldItem.isClearanceRequest && !fieldItem.disable && 'method-left'
          "
          >VAT/EORI/POA：</span
        >
        <AutoComplete
          ref="ClearanceAutoComplete"
          :formModel="formModel"
          :fieldItem="{
            id: 'customsClearanceId',
            placeholder: '请选择VAT/EORI/POA',
            multiple: true,
            showCreate: fieldItem.hideAdd ? false : true,
            ...
            isDisabled: fieldItem.isDisabled,
            handleCreate: (row) => handleUpLoad(row, 'ClearanceAutoComplete'),
          }"
          defaultKey="id"
          v-if="!fieldItem.disable"
        />
        ...
        <TooltipOver
          v-else-if="handleClearance(formModel.customsClearanceId)"
          :content="handleClearance(formModel.customsClearanceId)"
          class="clearance-content"
          refName="tooltipOver"
          :style="{ maxWidth: fieldItem.maxWidth || '320px' }"
        />
      </div>
      <div v-else>
        <slot
    ...
    getList(params) {
      ...
      return typeList[this.fieldItem.type]
        ...
        .then((res) => {
          ...
          if (!Object.keys(this.contactsData).length || !this.contactsData.id) {
            const data = this.formModel[this.fieldName];
            if (data) {
              this.contactsData = { ...data, ...this.getCustomData(data) };
            }
          ...
          if (
            (!Object.keys(this.contactsData).length || !this.contactsData.id) &&
            res.rows &&
            res.rows.length &&
