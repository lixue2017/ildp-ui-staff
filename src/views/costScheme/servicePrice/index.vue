  ...
  <div class="app-container">
    <ComTabs :tabsConfig="tabsConfig">
      <!-- <template v-slot:btnOptions="{ slotProps }">
        <el-button
          class="btn-color"
          :size="slotProps.btn.size"
          :type="slotProps.btn.type"
          :icon="slotProps.btn.icon"
          @click="slotProps.btn.handleClick"
          >{{ slotProps.btn.table }}</el-button
        >
      </template> -->
                    ...
                    <div class="btn">
                      <template v-for="(item, index) in tableConfig.statusList">
                        <el-button
                          ...
                          :key="index"
                          v-if="item.status.includes(slotProps.row.status)"
                          v-debounce
                        >
                          {{ item.text }}
                        </el-button>
...
<script>
import { servicePrice } from "../components/model.js";
  ...
  data() {
    return {
      ...
      searchFormConfig: {
        ...
        searchForm: true,
        labelWidth: "100px",
        lists: servicePrice.formLists, // 表单渲染列表
        formRules: {}, // 表单校验规则
      },
      tableConfig: {
      ...
      mixinTableColArr: [servicePrice.tableLists],
    };
  },
  mounted(){
    this.searchFromData={
