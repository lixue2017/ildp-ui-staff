    ...
    <ComTabs :tabsConfig="tabsConfig" :beforeLeave="beforeLeave">
      ...
      <template v-slot:slotTabsRight>
        ...
        <el-checkbox
          ...
          :disabled="
            data.formModel.orderBusinessType == 2 ||
            data.formModel.orderBusinessType == 3
              ? true
              : false
          "
          class="checkbox"
...
export default {
  ...
  watch: {
    ...
    data: {
      deep: true,
      handler: function (newVal, oldVal) {
        const { orderBusinessType } = newVal.formModel;
        if (orderBusinessType == 2 || orderBusinessType == 3) {
          //orderBusinessType   2仅拖车  3拖车+报关  如果为仅拖车与拖车+报关时，自动勾选上门提货且无法修改
          this.formModel.isHomeDelivery = true;
        }
      },
    },
  },
