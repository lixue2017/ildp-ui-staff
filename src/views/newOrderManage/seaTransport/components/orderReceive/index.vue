    ...
    <template v-slot:content>
      ...
      >
        <template v-slot:tips>
          <span v-if="type == 4" class="form-tips"
        ...
        <template v-slot:overseasAgency="slotProps">
          <AutoComplete
            ...
            :fieldItem="{
              ...slotProps.fieldItem,
              className: slotProps.formModel.overseasAgencyType == 1 ? '' : '',
              disabled:
                slotProps.formModel.overseasAgencyType == 1 ? false : false,
            }"
            :additionalParam="{
  ...
  methods: {
    submitFormRequest(formModel) {
      ...
      const params = {
        ...
        // bookingSpaceProxy:
        //   this.type == 1 ? formModel.deptId : formModel.bookingSpaceProxy, //传统整柜传公司ID
      };
      ...
      getSalesman({
        ...
        this.formConfig = {
          ...
          formModel: {
            ...
            customerId,
            deptId,
            // bookingSpaceProxy: type == 1 ? deptName : "",
            transactionWarehousePlatformType, //交货仓拼接好字段
            transactionWarehouseCode,
            transactionWarehouseName,
            insuredAmount: insuredAmount || insuranceAmount,
            WEBproxyTypeIsPublic:
              proxyTypeDicts[0] && proxyTypeDicts[0].isPublic, //仅用于动态配置
            WEBcustomerIds: proxyTypeDicts[0] && proxyTypeDicts[0].customerIds, //仅用于动态配置
    ...
    handleBookingChange(val) {
      if (val) {
        this.$refs.quotationSheetRef?.changeFocusRequest();
      }
    },
    handleChange(row, id) {
      if (
        this.type == "1" ||
        (this.type == "2" &&
          ["overseasAgencyType", "isWarehousing", "proxyType"].includes(id))
      ) {
        //1整柜  2散货
