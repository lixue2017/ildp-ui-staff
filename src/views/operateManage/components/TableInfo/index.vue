<template>
  <div class="detail-table">
    <template v-for="(item, index) in table">
      <div :key="index" v-if="(showTable || [0, 1, 2]).includes(index)">
        <div class="title">
        ...
        >
          <template v-slot:customRow="{ slotProps }">
            <template v-if="slotProps.prop === 'customsDeclarationType'">
              <div
                class="tag delete"
                v-if="slotProps.row.deleted && !isCabinet"
              >
                删
              </div>
              <div
            ...
            <template v-if="slotProps.prop === 'packNum'">
              <DiffMark
                ...
                :showCurrentVal="false"
                :showVerifyVal="false"
                v-if="slotProps.row.warehousingBoxNum !== null && !isCabinet"
              />
            ...
            <template v-if="slotProps.prop === 'length'">
              <DiffMark
                ...
                :showCurrentVal="false"
                :showVerifyVal="false"
                v-if="slotProps.row.warehousingLength !== null && !isCabinet"
              />
            ...
            <template v-if="slotProps.prop === 'width'">
              <DiffMark
                ...
                :showCurrentVal="false"
                :showVerifyVal="false"
                v-if="slotProps.row.warehousingWidth !== null && !isCabinet"
              />
            ...
            <template v-if="slotProps.prop === 'height'">
              <DiffMark
                ...
                :showCurrentVal="false"
                :showVerifyVal="false"
                v-if="slotProps.row.warehousingHeight !== null && !isCabinet"
              />
            ...
            <template v-if="slotProps.prop === 'goodsTotalWeight'">
              <DiffMark
                ...
                :showVerifyVal="false"
                v-if="
                  slotProps.row.warehousingTotalWeight !== null && !isCabinet
                "
              />
            ...
            <template v-if="slotProps.prop === 'totalVolume'">
              <DiffMark
                ...
                :showVerifyVal="false"
                v-if="
                  slotProps.row.warehousingTotalVolume !== null && !isCabinet
                "
              />
            ...
            <template v-if="slotProps.prop === 'pickAddress'">
              <el-row class="ellipsis-text">
                <span>{{
                  slotProps.row.cusConsignee &&
                  slotProps.row.cusConsignee.address
                }}</span>
              </el-row>
            </template>
        ...
        <div class="grand-total" v-if="[0, 1].includes(index)">
          ...
          >
            ...
            <DiffMark
              ...
              :verifyVal="
                gross.prop || (index === 0 && formModel.encasementWay == 0)
                  ? ''
                  : index === 1
                  ? verifyTotal[gross.propBtm] || ''
                  : gross.totalBtm || ''
              "
              :decimal="gross.decimal"
              :grossTotal="!!gross.totalBtm || !!verifyTotal[gross.propBtm]"
            />
          </div>
        </div>
      </div>
...
export default {
  ...
  created() {
    var { freightType, orderType } = this.formModel;
    if (freightType == 0) {
      this.isCabinet = true;
    }
    if (orderType === 3) {
      ...
      //空运
      this.table = [kyLclGoodsInfo, predictionLclGoodsInfo, lclPickGoodsInfo];
    } else if (orderType === 5) {
      //拖车
      if (freightType == 0) {
        this.table = [fclGoodsInfo, predictionFclGoodsInfo, fclPickGoodsInfo];
  ...
  methods: {
    handleMaintain() {
      ...
      this.$refs.maintainTransition.show(freightType, orderType);
    },
        ...
        reqs[index].httpRequest(params).then((res) => {
          ...
          if (index === 1) {
            ...
            res.rows = res.rows.map((item) => {
              const totalVolume = orderType == 2 ? Number(
                (
                  Number(item.volume || 0) *
                  Number(item.packNum || item.boxNum || 0)
                ).toFixed(2)
              ) : item.totalVolume;
              const totalWeight = orderType == 2 ? Number(
                (
              ...
              return {
                ...
                calcPackNum: item.packNum || item.boxNum || 0,
              };
            });
          }
          if (reqs[index].type) {
    ...
    getResult(index, { total }) {
      ...
      if (this.grossTotal[index].code === 200) {
        ...
        if (index === 0 || index === 1) {
          console.log(`${this.table[index].title}----------------------------`);
          console.log(this.table[index].grossTotal);
        }
      }
    },
    tableAutoHeight(idx) {
