<template>
  <div class="dynamic">
    <div class="title">线索动态</div>
    <div class="operation">
      <el-button
        type="primary"
        size="small"
        @click="handleFollowUp"
        v-if="!isCommunal && hasBtnPermits(['clueManage:salesClue:followUp'])"
      >
        <i class="el-icon-chat-dot-round"></i><span>写跟进</span>
      </el-button>
      <el-checkbox v-model="onlyFollow" @change="refreshTable"
        >只看跟进</el-checkbox
      >
    </div>
    <div class="dynamic-context">
      <template v-for="(item, i) in list">
        <div class="" :key="`dynamic${i}`">
                ...
                <div class="context-center">
                  <div>
                    ...
                    <span class="context-time"
                      >{{ { 2: "任务", 3: "跟进" }[item.type]
                      }}{{ handleType(crmFollowType, item.followType) }}</span
                    >
                  </div>
                  <div v-if="item.type === 3" v-html="item.context"></div>
                  <div v-else>{{ item.context }}</div>
                  <div>来自线索：{{ item.businessName }}</div>
                </div>
                  ...
                  <div>
                    <template v-for="(btn, i) in handleBtn(btnList)">
                      <el-button
                        v-if="
                          !btn.hide &&
                          (!btn.showType || btn.showType.includes(item.type))
                        "
                        :key="i"
                        type="info"
                        size="small"
...
<script>
import { getSalesLeadsDynamicPage } from "@/api/clueManage/salesClue.js";
